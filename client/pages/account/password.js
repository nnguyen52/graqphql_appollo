...
const Password = () => {
  ...
  const handleResetPassword = async (values, { setErrors }) => {
    ...
    await changePassword({
      ...
      update(cache, { data }) {
        console.log('after changepass: ', data);
        ...
        {({ isSubmitting }) => (
          <Form>
            <Box sx={{ display: 'flex', gap: '.5em', flexDirection: 'row' }}>
              ...
              <InputField name='confirmNewPassword' type='password' label='Confirm new password' />
              <LoadingButton disabled={message} loading={isSubmitting} type='submit'>
                Update password
              </LoadingButton>
            </Box>
            {(isSubmitting || loadingChangePassword) && <LinearProgress />}
